...
$(function() {
    ...
    var url =  config.URL + 'phonebook/vtushare';
    function get_vtu_distribution() {
        return $.ajax({
            url: url,
            data: param,
            dataType: 'JSON'
    ...
    var outlet_distribution = get_vtu_distribution();
    
    outlet_distribution.success(function(distribution_data) {
        $('#vtu_share').highcharts({
            chart: {
    ...
    url =  config.URL + 'phonebook/productsourceshare';
    function get_pshare_distribution() {
        return $.ajax({
            url: url,
            data: param,
            dataType: 'JSON'
    ...
    var outlet_pshare_distribution = get_pshare_distribution();
    outlet_pshare_distribution.success(function(distribution_data) {

        $('#productsource_share').highcharts({
            chart: {
                plotBackgroundColor: null,
                plotBorderWidth: null,
                plotShadow: false
    ...
    function getOutlets(param) {
        ...
        var all_outlets_url = config.URL + "phonebook/paginatedphonebook";
    ...
    function createTableHead () {
        ...
        thead += '<tr>';
        thead += '<th> Name</th>';
        /*thead += '<th width="10%"> Outlet Name </th>';*/
        thead += '<th width="20%"> Contact Phone Number </th>';
        thead += '<th width="20%"> Alternate Number </th>';
        thead += '<th width="20%"> VTU Number </th>';
    ...
    function createTableBody (data) {
        ...
        for(var i = 0; i < data.length; i++) {
            ...
            tbody += '<td>' + data[i].Outlet.contactalternatenumber + '</td>';
            tbody += '<td>' + data[i].Outlet.vtunumber + '</td>';
